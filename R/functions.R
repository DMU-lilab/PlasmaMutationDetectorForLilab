...
PrepareLibrary = function(info.dir='Info/',
  ...
  if (file.exists(snp.file) & file.exists(tbi.file)) {
    ...
    print(paste0("Reading bed file " , bed.filename))  # change here by ywliao, 2018.09.21
    bed = read.delim(bed.file, header=FALSE) # original amplicons
    # Build index of all positions
    pos_ind = NULL
    for(i in 1:nrow(bed)) pos_ind = unique(c(pos_ind,paste0(bed[i,1],':',bed[i,2]:bed[i,3]))) #DZQ 28/11/2017 add unique,to remove duplicates in chrpos

    # original positions for each amplicons
    pos_ranges = GRanges(bed[,1], IRanges(bed[,2], bed[,3]))
    # Build our snp selection
    print(paste0("Reading snp file ", snp.file)) # change here by ywliao, 2018.09.21
    chr_line <- as.integer(system(paste0("less ",snp.file, " | head -n 100 | grep chr | wc -l "), intern = T)) # change here by ywliao, 2018.09.27, seqname have chr or not
    if(chr_line > 5){
       vcf = readVcf(snp.file, "hg38", param=ScanVcfParam(which=GRanges(bed[,1], IRanges(bed[,2], bed[,3]))))
    }else{
       vcf = readVcf(snp.file, "hg38", param=ScanVcfParam(which=GRanges(gsub("chr", "", bed[,1]), IRanges(bed[,2], bed[,3]))))
    }
    snp = cbind(info(vcf), as.data.table(rowRanges(vcf))) # extract vcf information   # change here by ywliao, 2018.08.29
    if("AF" %in% colnames(snp)){
    ...
    }else if("CAF" %in% colnames(snp)){  # change here by ywliao, 2018.09.20, some snp file has CAF column
        snp$AF = suppressWarnings(sapply(snp$CAF, function(x)as.numeric(x[2])))
        snp = snp[union(which(snp$AF>1e-4), which(is.na(snp$AF))),]
    }
    if(chr_line > 5){ # change here by ywliao, 2018.09.27, seqname have chr or not
      pos_snp = paste0(snp$seqnames,':',snp$start) # build snp positions
    }else{
...
BuildCtrlErrorRate = function(ctrl.dir='Plasma ctrl/',bai.ext='.bai',pos_ranges.file=NULL,hotspot.file=NULL,
  ...
  for (g in ind.g) {
    ...
    for (j in colnames(All.plus))
      if (is.numeric(All.plus[,j]) & j!='start') {
        ...
        All.plus[is.nan(All.plus[,j]),j] = 0  #changed here by ywliao, 2018.08.29
        Y.plus[is.na(Y.plus[,j]),j] = 0
        Y.plus[is.nan(Y.plus[,j]),j] = 0 #changed here by ywliao, 2018.08.29
        All.plus[,j] = All.plus[,j] + Y.plus[,j]
      }
    All <- All.plus  #changed here by ywliao, 2018.08.29
  }
  ...
  #X = X[ok,] #### change here by ywliao, 2018.09.20
  X = X[ok &!not.ok,] #### ensure to select positions in defined genomic ranges, ensure to remove SNP positions and background error rate for SNPs (E0) and INDELS (E0indel) in samples, change here by ywliao, 2018.09.20
  #MAJ = MAJ[ok,]
  MAJ = MAJ[ok & !not.ok,] #change here by ywliao, 2018.09.20
  All = All[ok & !not.ok,]
  ...
  tt <- which(N0 > 5000) # Remove genomic positions which are not covered in all control samples or with sum of depths < 5,000x and which ref is NULL #changed here, by ywliao, 2018.08.29
  MAJ <- MAJ[tt,] #changed here, by ywliao, 2018.08.29
  All <- All[tt,]  #changed here, by ywliao, 2018.08.29
  X <-  X[tt,]  #changed here, by ywliao, 2018.09.23
  N0 <- N0[tt]  #changed here, by ywliao, 2018.09.26
  E0 <- E0[tt]   #changed here, by ywliao, 2018.09.26
  ...
  write.table(na.omit(background_error_rate), file=backround_error_file, row.names=FALSE, col.names=TRUE)  #changed here, by ywliao, 2018.08.29
...
DetectPlasmaMutation = function(patient.dir='./',patient.name=NULL,pos_ranges.file=NULL,ber.ctrl.file=NULL,bai.ext='.bai',
                               n.trim=2, cov.min=0,force=FALSE,show.more=FALSE,
  ...
  #change here, n.trim change default to 2, by ywliao 2018.09.04
  ...
  for (i in 1:length(patient.files)) {
    ...
    sample = sample[which(!is.na(sample$MA) & sample$MA > 1),] ### remove positions which mutation reads haven't been read, or only one read support mutation
#,changed here by ywliao, 2018.08.29
    ...
    samplenoise$Revert = F
    save(samplenoise, file= "~/samplenoise.Rdata")
    for (q in 1:nrow(samplenoise)) {
      ...
      if (as.character(samplenoise$Basemut[q])==as.character(samplenoise$REF[q])) {
        ...
        samplenoise$Revert[q] = T #change here by ywliao, 2018.09.23
      }
    }
    ...
    type.snv.revert = (samplenoise$type == 'SNV' & samplenoise$Revert == T)
    ind = which(type.snv.revert)
    samplenoise[ind,'Freq'] = samplenoise[ind,'maj']/samplenoise[ind,'cov'] # changed by ywliao, 2018.09.23
    ind = which(!type.snv)
    ...
    if(length(which(!is.na(uppers))) > 0){  #change here by ywliao 2018.09.19
      for (q in uppers) {
